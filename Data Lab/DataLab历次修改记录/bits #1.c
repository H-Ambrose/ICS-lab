 ...
 */
int zeroByte(int x) {
    /*will it must consider under zero?*/
    int x1 = !(x & 0xff);
    int x2 = !(x & (0xff << 8));
    int x3 = !(x & (0xff << 16));
    int x4 = !(x & (0xff << 24));
...
int negPerByte(int x) {
    ...
    x = x >> 8;
    int x4 = x & 0xff;
    return (~x1 + 1) | (~x2 + 1) << 8 | (~x3 + 1) << 16 | (~x4 + 1) << 24;
}
/*
 ...
 * twos2ones - convert from two's complement to one's complement,
             ...
             -      2. take A as an normal two's complement integer and remember
 its value
             -      3. produce as result an integer B such that when B's bits
             -         are interpreted as one's complement, the value of B
 equals
             -         the value of A
             -      4. return B
             -  NOTE:
             -      1.Since morden computers use two's complment, B's actual
 value in the computer
             -        may differ from A's. Remember what we want is essentially
 the bit representation
             -        not the value.
             -      2.As you may know, the ranges of two's complement and one's
 complement are not exactly the same.
             -        You should assume all the testing arguments can be both
 represented with two's and one's.
             -      3.There are two representations of 0 in one's complment,
 please *only* return 0.
...
int twos2ones(int x) {
    int flag = x >> 31;
    int one = ~1 + 1;
    return (flag & (x + one)) | ((~flag) & x);
}
/*
 * ones2twos - convert from one's complement to two's complement,
             - the conversion should work as follows:
             -      1. given as argument an integer A
             -      2. keep A's bit representation, calculate the value it
 represents under one's complement
             -      3. produce as result an integer B such that B's value in
 two's complement
             -         equals the above value
             -      4. return B
             -  NOTE:
             -      1.Since morden computers use two's complment, A's actual
 value in the computer
             -        may differ from B's. Remember what we want is essentially
 the bit representation
             -        of A not the value.
 ...
 *   Legal ops: Any long long/unsigned long long operations incl. ||, &&. also
 if, while
...
unsigned long long float2double(unsigned long long uf) {
    unsigned long long exp = (uf << 33 >> 54) - (1 << 7) + (1 << 10);
    unsigned long long s = uf >> 31;
 ...
 *   Legal ops: Any long long/unsigned long long operations incl. ||, &&. also
 * if, while Max ops: 10 Rating: 2
 */
unsigned long long double_abs(unsigned long long uf) {
    unsigned long long nan = 0x7ff0000000000001;
    if(uf >= nan)
        return uf;
    else
        return uf << 1 >> 1;
...
unsigned long long double_half(unsigned long long uf) {
    ...
    unsigned long long exp = uf << 1 >> 53;
    unsigned long long frac = uf << 12 >> 12;
    if(exp == 0)
        frac = frac >> 1;
    else if((exp + 1) == (1 << 11))
        return uf;
 ...
 *   NOTE: For simplicity, ignore the sign bit and treat all arguments as
 * positive. Hint: be careful of rounding issues, refer to
 *   http://pages.cs.wisc.edu/~markhill/cs354/Fall2008/notes/flpt.apprec.html
 * for more details Legal ops: Any long long/unsigned long long operations incl.
 * ||, &&. also if, while Max ops: 100 Rating: 5
 */
unsigned long long double_add(unsigned long long f1, unsigned long long f2) {
    if(f1 > f2) {
        unsigned long long t = f1;
        f1 = f2;
        f2 = t;
    }

    unsigned long long exp1 = f1 << 1 >> 53;
    unsigned long long exp2 = f2 << 1 >> 53;
    if(exp1 == 0x7ff || exp2 == 0x7ff) return 0;
    unsigned long long mve = exp2 - exp1;
    unsigned long long frac = f1 << 12 >> 12;
    if(frac & (1 << (mve - 1))) {
        if(frac & ((1 << (mve - 1)) - 1)) frac = (frac >> mve) + 1;
    }
    else
        frac = frac >> mve;
    frac += f2 << 12 >> 12;
