...
/* this is the function defined by 朱子仪*/
...
void eval(char *cmdline) {
    ...
    /*PART1:重定向*/
    int stdi, stdo;
    stdi = dup(STDIN_FILENO); /*save STDIN_FILENO and STDOUT_FILENO*/
    ...
    if(tok.infile != NULL) {
        ...
        dup2(infd, STDIN_FILENO); /*I/O redirection (input). */
    }
    if(tok.outfile != NULL) {
        outfd = open(tok.outfile, O_RDWR, 0);
        dup2(outfd, STDOUT_FILENO); /*I/O redirection (output). */
    }
    ...
    if(tok.builtins == BUILTIN_NONE) { /* 不是 built-in command */
        sigprocmask(SIG_BLOCK, &mask, NULL);

        if((pid = fork()) == 0) {
            sigprocmask(SIG_UNBLOCK, &mask, NULL);
            if(setpgid(0, 0) == -1) sio_puts("create child process failed.\n");
            ...
            exit(0); /*防止execve不能正常运行造成后续无关步骤被执行*/
        }
        else {
    ...
    if(infd != -1) close(infd);
    if(outfd != -1) close(outfd);
    /*如果infd或者outfd为-1，说明没有进行重定向，不需要close*/
    return;
}
...
/*
 * builtin_command
 * 满足模块化要求，将处理内置command的部分单独拿出来，包括quit,jobs,fg,bg
 */
void builtin_command(struct cmdline_tokens *tok) {
    struct job_t *job;
    if(tok->builtins == BUILTIN_QUIT)
        ...
        /*this STDOUT_FILENO may has been redirection*/
    }
    else {
        int jid;
        if(tok->argv[1][0] == '%')
